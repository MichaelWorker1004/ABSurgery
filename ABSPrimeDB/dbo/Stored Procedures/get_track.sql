...
select  convert(char(8),pre_send,1),     convert(char(8),pre_receive,1),
	...
	convert(char(8),card_receive,1), convert(char(8),reg_receive,1), 
	convert(char(8),let_send,1),     convert(char(8),let_receive,1), 
	convert(char(8),fee_receive,1),  convert(char(8),fee_bounce,1),
	fee_amount, track.note, trackno ,
	convert(char(8),sig_receive,1),  convert(char(8),cme_receive,1),isnull(trans_id ,''),
	isnull(a.status,0),isnull(b.status,0) ,web_note
from track 
left join app_response a on a.candidate=track.candidate and a.exam_type=track.exam+track.type and a.app_id=0
left join app_response b on b.candidate=track.candidate and b.exam_type=track.exam+track.type and b.app_id=1
