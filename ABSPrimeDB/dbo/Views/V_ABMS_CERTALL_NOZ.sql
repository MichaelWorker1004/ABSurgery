CREATE VIEW [dbo].[V_ABMS_CERTALL_NOZ] AS
select * from (
	select
		exam_pass.UserId,
		exam_pass.candidate,
		...
		IIF(exam_pass.exam in ('G','V'),'General Certificate','Subcertificate') CertificateType,
		case exam_pass.exam
			when 'G' then 'S' 
			when 'C' then 'SCC' 
			when 'H' then 'HS' 
		...
		end CertificationExpireDateOld,
		convert(varchar(10),getdate(),20) CertificationExpireDate,
		/*case 
		...
		,'N','Y') iscertified,
		exam_pass.id,
		IIF(top_exam_pass.UserId is null,0,1) istop
	FROM exam_pass
		LEFT JOIN (
			SELECT max(exam_pass.durationtype_order) durationtype_order,max(isnull(exam_pass.date,reverification_date)) date,exam_pass.UserId,exam_pass.exam from exam_pass 
				INNER JOIN (SELECT max(durationtype_order) durationtype_order, UserId,exam from exam_pass group by UserId, exam) a ON a.durationtype_order= exam_pass.durationtype_order AND a.UserId = exam_pass.UserId AND a.exam = exam_pass.exam 
			GROUP BY exam_pass.UserId,exam_pass.exam,exam_pass.durationtype_order) top_exam_pass ON top_exam_pass.UserId=exam_pass.UserId and top_exam_pass.exam=exam_pass.exam and top_exam_pass.durationtype_order=exam_pass.durationtype_order and top_exam_pass.date=isnull(exam_pass.date,exam_pass.reverification_date)
		...
		left join (select max(certificate) certificate,min(date) date,UserId,exam from exam_pass where isnull(expiration,0)!=0 group by UserId,exam) initial_cert on initial_cert.UserId=exam_pass.UserId and initial_cert.exam=exam_pass.exam 
		...
		left join exam_eligibility on exam_pass.UserId=exam_eligibility.UserId and exam_pass.exam=exam_eligibility.exam and exam_eligibility.admissible='Y' and exam_eligibility.type not in ('X','P')
	where exam_pass.type not in ('X','S','P','I') and exam_pass.exam='Z' and DATEDIFF(day,getdate(),cast('01/01/'+cast(exam_pass.expiration+1 as varchar(4)) as datetime))>0
) info