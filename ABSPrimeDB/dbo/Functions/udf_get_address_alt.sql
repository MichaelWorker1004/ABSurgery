CREATE FUNCTION udf_get_address_alt(@code varchar(6),@field varchar(8))
RETURNS varchar(204)
AS
BEGIN
...
SELECT TOP 1 @org1=isnull(title,''),@org2=ltrim(rtrim(isnull(institution,''))),
...
FROM address 
where len(rtrim(isnull(street1,'')+isnull(street2,'')))>0 and code=@code
order by mail desc, modified desc
